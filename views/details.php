...
{
	global $conn, $quote, $alias_prefix, $prefix_internal_table, $submit_buttons_ar, $normal_messages_ar, $dadabik_main_file, $show_top_buttons, $separator_display_select_multiple, $separator_display_linked_field, $function_link_to_record, $site_url, $export_to_pdf_feature, $dir_templates, $enable_edit, $show_edit_button_in_details, $custom_button_ids_prefix;
	...
	if ( $export_to_pdf_feature == 1) {
		...
		$templates_menu = '<select name="pdf_template" id="pdf_template" style="visibility:hidden;display:inline-block;"><option value="" disabled selected>'.$normal_messages_ar['choose_pdf_template'].'</option><option value="">'.$normal_messages_ar['no_pdf_template'].'</option>';
		$templates_menu_2 = '<select name="pdf_template" id="pdf_template_2" style="visibility:hidden;display:inline-block;"><option value="" disabled selected>'.$normal_messages_ar['choose_pdf_template'].'</option><option value="">'.$normal_messages_ar['no_pdf_template'].'</option>';
	...
	if ($output === 'web_details'){
		...
		if ( $show_top_buttons == 1 && $onlyform === 0) {
			...
			if ($is_items_table === 1 && ($just_inserted === 0 || $from_back_edit_details_insert_delete_items_table === 1)){
				...
				$details_table .= '<input type="button" class="button_form go_back" value="'.$submit_buttons_ar['go_back'].'" onclick="javascript:document.location=\''.$dadabik_main_file.'?tablename='.urlencode($master_table_name).'&function='.$master_table_function.'&where_field='.urlencode($master_table_where_field).'&where_value='.urlencode(unescape($master_table_where_value)).'&from_back_edit_details_insert_delete_items_table=1\'">';
			}
			elseif ($just_inserted === 0  || $from_back_edit_details_insert_delete_items_table === 1){
		
				$details_table .= '<input type="button" class="button_form go_back" value="'.$submit_buttons_ar['go_back'].'" onclick="javascript:document.location=\''.$dadabik_main_file.'?function=search&tablename='.urlencode($table_name).'\'">';
			}
			
			if ($enable_edit == "1" && $show_edit_button_in_details === 1){
		
                $details_table .= '&nbsp;<input type="button" class="button_form" value="'.$submit_buttons_ar['edit'].'" onclick="javascript:document.location=\''.$dadabik_main_file.'?function=edit&tablename='.urlencode($table_name).'&where_field='.urlencode($where_field).'&where_value='.urlencode(unescape($where_value));
			...
			if ( $export_to_pdf_feature == 1) {
				...
				$details_table .= '&nbsp;<input type="button" class="button_form" value="PDF" id="pdf_button" onclick=" document.getElementById(\'pdf_template\').style.visibility = \'visible\';document.getElementById(\'pdf_button\').style.display = \'none\'; document.getElementById(\'produce_pdf_button\').style.visibility = \'visible\';">'.$templates_menu.' <input type="submit" id="produce_pdf_button" value="'.$normal_messages_ar['produce_pdf'].'" style="visibility:hidden;display:inline-block;">';
			}
			$details_table .= '</form>';
			
		}
		$details_table .= $buttons_html;
	...
	for ($i=0; $i<$count_temp; $i++){
		...
		if ($fields_labels_ar[$i][$presence_field_to_check] == "1"){
			...
			if ($output === 'web_details' || $output === 'PDF'){
				...
				$details_table .= '<label class="detail_page_label" for="'.$field_name_temp.'">'.$fields_labels_ar[$i]["label_field"].'</label>&nbsp;&nbsp;&nbsp;';
			}
	...
	if ($output === 'web_details'  && $onlyform === 0){
		...
		if ( $export_to_pdf_feature == 1) {
			...
			$details_table .= '&nbsp;<input type="button" class="button_form" value="PDF" id="pdf_button_2" onclick=" document.getElementById(\'pdf_template_2\').style.visibility = \'visible\';document.getElementById(\'pdf_button_2\').style.display = \'none\'; document.getElementById(\'produce_pdf_button_2\').style.visibility = \'visible\';">'.$templates_menu_2.' <input type="submit" id="produce_pdf_button_2" value="'.$normal_messages_ar['produce_pdf'].'" style="visibility:hidden;display:inline-block;">';
		}
