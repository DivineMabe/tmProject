...
// check if the field names of a table or the table name contain blank spaces, dots or other not allowed charaters, if no table in input, check all the tables, if a field name in input, just check that field name
{
	...
	if (!is_null($field_name) ){
        ...
        if ($only_installed === 1){
            ...
            $sql = "SELECT ".$quote."name_field".$quote.",".$quote."table_name".$quote."  FROM ".$quote.$prefix_internal_table."forms".$quote;
...
// from "2000-12-15" to "15 Dec 2000"
{
	...
	switch ($date_format){
		case "literal_english":
			$date=@date("M j, Y",mktime(0,0,0,$temp_2_ar[1],$temp_2_ar[2],$temp_2_ar[0])).$time_part;
			break;
		case "latin":
			$date = $temp_2_ar[2].$date_separator.$temp_2_ar[1].$date_separator.$temp_2_ar[0].$time_part;
			break;
		case "numeric_english":
			$date = $temp_2_ar[1].$date_separator.$temp_2_ar[2].$date_separator.$temp_2_ar[0].$time_part;
			break;
	} // end switch
	return $date;
}
...
function get_valid_field_name($name)
{
    ...
    $characters_to_change = array(' ', '.', ';', '\\', '\'', '"', $quote, chr(0), $alias_prefix, $null_checkbox_prefix, $select_checkbox_prefix, $select_type_select_suffix, $field_button_hint_container_id_prefix, $custom_button_ids_prefix);
