...
// global: ...
{
	...
	if ($results_display_mode === 'list'){
		...
		$order_by_form .= '&nbsp;&nbsp;'.$normal_messages_ar['sort_by'].': <select name="order" style="display:inline-block">';
		...
		$order_by_form .= '</select>';
	
		$order_by_form .= ' <select name="order_type" style="display:inline-block">
	<option value="ASC"'.( ($order === 'ASC') ? ' selected':'').'>ASC</option>
	<option value="DESC"'.(($results_display_mode === 'DESC') ? ' selected':'').'>DESC</option></select> <input type="submit" value="'.$normal_messages_ar['sort'].'"></form>';
		...
		if ($unique_field_name != "" && $unique_field_name != NULL && $onlygrid === 0){ // exists a unique field
		    ...
		    $results_table .= "<th class=\"zeropad\"></th>"; // skip the first column and for edit, delete and details
		...
		while ($records_row = fetch_row_db($res_records)){
			...
			for ($i=0; $i<$count_temp; $i++){
				...
				if ($fields_labels_ar[$i]["present_results_search_field"] == "1" && $show_column[$fields_labels_ar[$i]["name_field"]] === 1){ // the user want to display the field in the search results page
					...
					$results_table .= '<td class="results_grid_cell" data-live-edit data-source="'.$table_name.'" data-row="'.$where_value.'" data-field="'.$fields_labels_ar[$i]["name_field"].'"  data-type="'.$fields_labels_ar[$i]["type_field"].'">'; // start the cell
