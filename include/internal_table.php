...
$int_fields_ar[$ar_index][3] = "Text fields~text/textarea/rich_editor/Date and time fields~date/date_time/insert_date_time/update_date_time/List fields~select_single/select_single_radio/select_multiple_menu/select_multiple_checkbox/File fields~generic_file/image_file/Other~ID_user/unique_ID";
$int_fields_ar[$ar_index][4] = "<ul><li><b>text:</b> a text box</li><br/>
    ...
    <li><b>insert_date_time:</b> the current date and time will be automatically inserted into this field when you insert a new record in your table; an insert_date_time field is automatically excluded from the insert form. Note that the corresponding database field type must be date or datetime</li><br/>
    <li><b>update_date_time:</b> the current date and time will be automatically inserted into this field when you update a record in your table; an update_date_time field is automatically excluded from the insert and edit forms. Note that the corresponding database field type must be date or datetime</li><br/>
    <li><b>date:</b> a datepicker tool. Note that the corresponding database field type must be date</li><br/>
    <li><b>date_time:</b> a datetimepicker tool. Note that the corresponding database field type must be datetime</li><br/>
    ...
    <li><b>select_multiple_menu:</b> a customizable dropdown menu with the possibility to choose more than one option. Note that the corresponding database field type must be varchar.</li><br/>
    <li><b>select_multiple_checkbox:</b> a list of checkboxes, with the possibility to choose more than one. Note that the corresponding database field type must be varchar.</li><br/>
    <li><b>generic_file:</b> an input field which allows the user to browse his/her local file system and upload a file. You need to enable uploads (\$enable_uploads = 1) and specify the allowed file extensions in config.php. Note that the corresponding database field type must be varchar.</li><br/>
    <li><b>image_file:</b> the same as the above, but in this case DaDaBIK supposes that the file is an image and shows it when the record is displayed.</li><br/>
    <li><b>ID_user:</b> the username of the current user will be automatically inserted into this field when you insert a new record; an ID_user field MUST BE excluded from the insert form. Note that the corresponding database field type must be varchar.</li><br/> 
    <li><b>unique_ID:</b> a unique ID generated by using the PHP function unique_ID() in conjunction with microtime(), getmypid() and md5() will be automatically inserted in this field when you insert a new record into your table; a unique_ID field must be excluded from the insert form. Note that the corresponding database field type must be varchar(50). Probably you will NEVER use this field type, it can be useful just in very special cases; it is NOT the field type you should use for your primary key, autoincrement fields, for those fields you can just use TEXT.</li></ul>";
...
if ($orazio_edition === 1){
    ...
    $low_code_separator_text =  'Custom functions (LOW-CODE)<span id="confirmation_message_container"><div class="msg_error" id="alert_message"><h3>Here you can add your custom PHP/Javascript custom code.<br>For example you can add PHP code to make a field conditional according to some logic<br> or write a Javascript function that is fired when a particualr event occurs. <br><br>This section is <b>DISABLED</b> when you use an '.$orazio_name.' free account.<br>You need to <a href="index.php?function=deploy">publish and deploy</a> this application to enable it.</h3></div></span>';
}
...
$ar_index++;
$int_fields_ar[$ar_index][0] = "Custom display / required function:";
...
$int_fields_ar[$ar_index][3] = "50";
$int_fields_ar[$ar_index][4] = "Specify here the <b>name</b> of the function (if any) to be used to evaluate if this field needs to be displayed in the current form or not, for example according to the value of other fields.<br><br>The same function can also be used to determines if the field is <b>required</b> or not, overriding the parameter <i>Is the field a required one?</i> (see later in this form configurator). This function only affects insert end edit forms.<br><br>
...
The function name must start with <b>dadabik_</b> and you must write the function code in <b>/include/custom_functions.php</b>. The <b>/include/custom_functions.php</b> contains all the instructions to write it correctly.";
