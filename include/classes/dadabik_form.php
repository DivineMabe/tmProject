...
class dadabik_form
{
	...
	function render()
	{
		...
		if (!is_null($this->properties['submit_button'])){
			echo '<input class="button_form" type="submit" value="'.($this->properties['submit_button']).'">';
		}
		...
		if (!is_null($this->properties['back_button'])){
			...
			echo ' <input class="button_form" type="button" onclick="javascript:document.location=\''.($back_button_url).'\'" value="'.($this->properties['back_button']).'">';
		}
