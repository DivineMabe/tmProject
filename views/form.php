...
// output: $form, the html tabled form
{
	...
	if (count($custom_buttons_to_display) > 0){
		foreach($custom_buttons_to_display as $button){
			...
			$buttons_html .= ' <input'.$id_attribute.' type="button" class="button_form custom_button" style="'.$button['style'].'" value="'.$button['label'].'"';
	...
	switch($form_type){
		...
		case "update":
			...
			if ( $show_top_buttons == 1 && $onlyform === 0) {
				...
				$form .= "<input class=\"button_form\" type=\"submit\" value=\"".$submit_buttons_ar[$form_type]." >>\"> ";
				...
				if ($is_items_table === 1){
		            ...
		            if ( $GLOBALS['show_back_button'] === 1){
					    $form .= ' <input type="button" class="button_form" value="'.$submit_buttons_ar['go_back'].'" onclick="javascript:document.location=\''.$dadabik_main_file.'?tablename='.urlencode($master_table_name).'&function='.$master_table_function.'&where_field='.urlencode($master_table_where_field).'&where_value='.urlencode(unescape($master_table_where_value)).'&from_back_edit_details_insert_delete_items_table=1\'">';
					}
					
					if ($form_type === 'update' && $GLOBALS['show_save_plus_back_button'] === 1){
		
						$form .= ' <input type="submit" class="button_form save_go_back" value="'.$submit_buttons_ar[$form_type].' + '.$submit_buttons_ar['go_back'].'" onclick="javascript:document.getElementById(\'dadabik_main_form\').action=document.getElementById(\'dadabik_main_form\').action + \'&go_back_after_saving=1\'">';
					}
					
					//$form .= ' <input type="button" class="button_form" value="'.$submit_buttons_ar['go_back'].'" onclick="javascript:document.location=\''.$dadabik_main_file.'?function=search&table_name='.urlencode($master_table_name).'\'">';
				...
				// end enterprise
				    if ( $GLOBALS['show_back_button'] === 1){
						$form .= ' <input type="button" class="button_form" value="'.$submit_buttons_ar['go_back'].'" onclick="javascript:document.location=\''.$dadabik_main_file.'?function=search&tablename='.urlencode($table_name).'\'">';
					}
					if ($form_type === 'update' && $GLOBALS['show_save_plus_back_button'] === 1){
						$form .= ' <input type="submit" class="btn btn-primary" value="'.$submit_buttons_ar[$form_type].' + '.$submit_buttons_ar['go_back'].'" onclick="javascript:document.getElementById(\'dadabik_main_form\').action=document.getElementById(\'dadabik_main_form\').action + \'&go_back_after_saving=1\'">';
					}
				// pro
				// enterprise
				}
				...
				if ($form_type === 'update' && $enable_insert == '1'){
				    $form .= ' <input type="submit" class="button_form save_go_back" value="'.$submit_buttons_ar['insert_as_new'].'" onclick="javascript:document.getElementById(\'dadabik_main_form\').action=document.getElementById(\'dadabik_main_form\').action + \'&function=insert\'">';
				}
		...
		case "search":
			...
			if ($select_operator_feature == "1"  && $onlyform === 0){
				$form .= "<div class=\"select_element select_element_search_boolean_operator\"><select name=\"operator\"><option value=\"and\">".$normal_messages_ar["all_conditions_required"]."</option><option value=\"or\">".$normal_messages_ar["any_conditions_required"]."</option></select></div>";
			} // end if
			else{
	...
	for ($i=0; $i<count($fields_labels_ar); $i++){
		if ( ($fields_labels_ar[$i][$field_to_ceck] == "1" || $fields_labels_ar[$i][$field_to_ceck] == "3") && ($fields_labels_ar[$i]['type_field'] !== 'insert_date_time' || $form_type !== 'insert') && ($fields_labels_ar[$i]['type_field'] !== 'update_date_time' || $form_type !== 'update' && $form_type !== 'insert' )  ) { // the user want to display the field in the form
			...
			$form .= '<label for="'.$field_name_temp.'">';
			...
			if (($fields_labels_ar[$i]['type_field'] === 'select_single' || $fields_labels_ar[$i]['type_field'] === 'select_single_radio' || $fields_labels_ar[$i]['type_field'] === 'select_multiple_menu' || $fields_labels_ar[$i]['type_field'] === 'select_multiple_checkbox') && $primary_key_field_field != ""){
				...
				*/
				...
				$temp_ar = build_dropdown_query($fields_labels_ar[$i], $form_type, $user_group_name, $show_edit_form_after_error, $details_row_to_pass, $filter_field_value_to_pass, $_POST_2);
				...
				case "update_date_time":
					if ($field_type_for_date === 'date_picker' || ($fields_labels_ar[$i]["type_field"] !== 'date' && $fields_labels_ar[$i]["type_field"] !== 'date_time')){
						...
						if ($fields_labels_ar[$i]["tooltip_field"] !== ''){
							...
							$form .= "<span class=\"tooltip_content\">".$fields_labels_ar[$i]["tooltip_field"]."</span></span>";
						}
                ...
                <td valign="top" >
                    ...
                    <div>
                        <div data-uploader-drop-zone class="dropzone">
                            <input id="" data-uploader-input type="file" style="display:none">
           ...
           if($upload_field_type === 'classic'){
						...
						if ($form_type == "update" or $form_type == "ext_update"){
							...
							if ($file_name_temp != ""){
                                ...
                                if ($fields_labels_ar[$i]["type_field"] === 'image_file' && $show_images_in_edit_form === 1){
                                    ...
                                    $image_to_display .= " src=\"index.php?r=".time()."&function=show_file&file_type=image_file&tablename=".urlencode($fields_labels_ar[$i]["table_name"])."&tablename_original=".urlencode($table_name)."&file_field_name=".urlencode($fields_labels_ar[$i]["name_field"])."&master_table_name=".urlencode($master_table_name)."&is_items_table=".urlencode($is_items_table)."&where_field=".urlencode($where_field)."&where_value=".urlencode(unescape($where_value))."\">";
				...
				case "textarea":
					...
					$form .= " cols=\"".$fields_labels_ar[$i]["width_field"]."\"";
				...
				case "select_multiple_checkbox":
					...
					if ($fields_labels_ar[$i]["type_field"] == "select_multiple_menu"){
						...
						$form .= " name=\"".$field_name_temp."[]\" size=".$size_multiple_select." multiple class=\"select_element";
	...
	if ($onlyform === 0){
	    ...
	    elseif ($form_type === 'insert' && $use_old_insert_form_button_label === 0 ){
	        $form .= "<div class=\"save_buttons_container\"><input type=\"submit\" class=\"button_form\" value=\"".$submit_buttons_ar['update']." >>\">";
	    }
	    else{
            $form .= "<div class=\"save_buttons_container\"><input type=\"submit\" class=\"button_form\" value=\"".$submit_buttons_ar[$form_type]." >>\">";
        }
