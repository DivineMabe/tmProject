...
<meta name="Generator" content="DaDaBIK 11.9 Elba - http://dadabik.com/">
...
if ($page_name === 'main' || $page_name === 'login'){
	switch($graphic_theme)
	{
		case 'black':
			echo '<link rel="stylesheet" href="css/theme_black.css';
			...
			break;
		case 'green':
			echo '<link rel="stylesheet" href="css/theme_green.css';
			...
			break;
		case 'blue':
			echo '<link rel="stylesheet" href="css/theme_blue.css';
			...
			break;
		case 'bluegray':
			echo '<link rel="stylesheet" href="css/theme_bluegray.css';
...
<?php } else{ ?>
<script src="include/custom_functions.js<?php if ( $force_reload_css_js === 1){ echo '?v='.rand(1,10000);} ?>"></script>
<?php } ?>
...
?>          
            ...
            }?></select> <select style="width:100px"  type="text" name="null_field_' + i + '"><option value="0">No</option><option value="1" >Yes</option></select> <select style="width:150px" type="text" name="pk_field_' + i + '"><option value="0"></option><option value="1" >Primary Key</option></select> <select style="width:180px"  type="text" name="extra_field_' + i + '"><option value=""></option><?php
            ...
            }?></select> <span style="width:150px;display:inline-block"><i>No</i></span> <input class="button_admin delete_field_button" id="' + tablename + '_' + i + '" type="button" value="DELETE FIELD"></div>');
<?php
} else{
?>
            // code repeated above, to fix
            // append the new field after the last one (i-1)
            $('#container_'+tablename + '_' + (i-1)).after('<div id="container_' + tablename + '_' + i + '"  data-tablename="' + tablename +'"><p><input type="text" style="width:150px" name="name_field_'  +i+ '" > <input style="width:150px"  type="text" name="type_field_' + i + '" > <select style="width:100px"  type="text" name="null_field_' + i + '"><option value="0">No</option><option value="1" >Yes</option></select> <select style="width:150px" type="text" name="pk_field_' + i + '"><option value="0"></option><option value="1" >Primary Key</option></select> <select style="width:180px"  type="text" name="extra_field_' + i + '"><option value=""></option><?php
            foreach($autoincrement_options_ar as $autoincrement_options_ar_el){
                echo '<option value="'.$autoincrement_options_ar_el.'" >'.$autoincrement_options_ar_el.'</option>';
            }?></select> <span style="width:150px;display:inline-block"><i>No</i></span> <input class="button_admin delete_field_button" id="' + tablename + '_' + i + '" type="button" value="DELETE FIELD"></div>');
<?php
...
<?php } ?>	
	...
	$("#include_tables_form").submit(function(e) {
		...
		if (pages_to_show === 'custom_pages'){
			...
			var tablename_to_update = '<?php echo $prefix_internal_table.'static_pages'; ?>';
                        ...
                        $.each(data.fields_show, function(key, val) {
                            ...
                            if (val == 1){
                               ...
                               $('label[for="'+key+'"]').show();
                                ...
                                document.getElementById('<?php echo $field_button_hint_container_id_prefix; ?>' + key).style.display = 'block';
                               
                               if (typeof document.getElementsByName('<?php echo $null_checkbox_prefix; ?>' + key)[0] !== 'undefined'){  
                               
                                 document.getElementsByName('<?php echo $null_checkbox_prefix; ?>' + key)[0].style.display = 'block';
                                }
                            }
                            else{
                                    $('#'+key+'_req').html('');
...
<?php } else{
        ...
        echo 'where_value_form_preview = \''.$_SESSION[$table_name.'_where_value_form_preview'].'\';';
...
?>
onload="hide_show_interface_configurator_fields(document.getElementById('e1'))"
<?php
} // end if
...
if ($page_name === 'main' || $page_name === 'login') {
    ...
    if ($orazio_edition === 1){
        ...
        echo '&nbsp;&nbsp;&nbsp;&nbsp;<table style="display:inline;" ><tr><td><a class="btn btn-lg btn-primary" href="admin.php">Edit this app</a>&nbsp;&nbsp;&nbsp;<a class="btn btn-lg btn-danger" href="index.php?function=deploy">Publish & Deploy this App</a>&nbsp;&nbsp;&nbsp;<a target="_blank" title="Ask a question, we\'ll get in touch very soon" class="btn btn-lg btn-info" style="background-color: #eff0f1 !important;border-color: #eff0f1 !important;text-decoration:none;" href="/contacts" title="Ask a question">Questions? Contact us</a> </td></tr></table>';
    }
...
elseif ($page_name === 'admin' || $page_name === 'interface_configurator' || $page_name === 'permissions_manager' || $page_name === 'tables_inclusion' || $page_name === 'db_synchro' || $page_name === 'datagrid_configurator' || $page_name === 'data') {
	...
	echo '</td><td align="right"> <font size="6">ADMIN AREA</font>&nbsp;<br/>';
	...
	echo ' href="admin.php?function=show_status&tablename='.urlencode($table_name).'">&nbsp;Status&nbsp;</a> - ';
...
if ($orazio_edition === 0){
	...
	echo ' href="admin.php?function=show_help&tablename='.urlencode($table_name).'">&nbsp;Help&nbsp;</a> - ';
	...
	echo ' href="admin.php?function=show_feedback&tablename='.urlencode($table_name).'">&nbsp;Feedback&nbsp;</a> - ';
}
	echo '<a';
	...
	echo ' href="admin.php?function=show_about&tablename='.urlencode($table_name).'">&nbsp;About/Upgrade&nbsp;</a>'; 
	...
	if (!isset($_SESSION['check_update'])){
		...
		$temp = file_get_contents('https://dadabik.com/last_release.php');
	...
	echo ' - <a href="'.$dadabik_main_file.'?function=search&tablename='.urlencode($table_name).'">Exit admin</a> - <a href="'.$site_url.'login.php?function=logout">Logout</a>';
	}
...
if ($menu_type === 'drop_down_menu' && $menu_drop_down !== ''){
...
<?php if (count($languages_ar) > 1){ ?>
<td align="right"><form method="post" class="css_form" style="margin:auto;"  action="<?php echo $dadabik_main_file; ?>?function=change_language">
 <div  class="select_element select_element_change_language"  ><select name="language" onchange="this.form.submit()">
<?php
foreach ($languages_ar as $value){
	echo '<option value="'.$value.'"';
...
<td>	
	<table class="table_interface_container_table_top_menu_admin_area">
